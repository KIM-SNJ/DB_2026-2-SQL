...
--가상컬럼명을 설정
--제품테이블에서 제조업체의 중복데이터를 제거하고 검색하시오.(단 제조업체를 대표업체로 가상컬럼명 변경)
select distinct 제조업체 as 대표업체 from 제품;

--제품테이블에서 제품명, 단가를 검색하되, 단가를 가격이라는 이름으로 출력하시오.
select distinct 제품명, 단가 가격 from 제품;

--제품테이블에서 제품명, 단가를 검색화되, 단가에 500원을 더해서 조정 단가라는 가상컬럼명으로 출력하시오.
...
--주문테이블에서 주문고객이 carrot이면서 수량이 15개 이상 주문한 주문고객, 주문제품, 수량, 주문일자를 출력하시오.
select 주문제품, 수량, 주문일자
    from 주문
    where 주문고객='carrot';
...
--주문고객이 apple이거나 수량이 20개 이상 주문한 주문고객, 주문제품, 수량, 주문일자를 출력하시오.
